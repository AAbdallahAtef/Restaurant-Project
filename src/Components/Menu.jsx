    ...
    <>
      ...
      <div className="container-fluid my-5">
        <div className="row justify-content-center gap-2">
          {categories.map((item) => (<button onClick={() => handleClick(item.idCategory, item.strCategory)} className={clickedButton === item.idCategory ? "col-11 col-sm-3 col-md-2 btn  rounded-5 fw-medium activeButton" : "col-11 col-sm-3 col-md-2 btn btn-outline-light fw-medium text-black border border-secondry rounded-5"} key={item.idCategory}>{item.strCategory}</button>))}
        </div>
      </div>

      <div className="container-fluid p-0 my-5">
        <div className='row justify-content-evenly gap-1 m-2 menu-items'>
          {menuItems.map(item => (
            <div className="col-12 col-sm-5 col-md-3 my-2 px-0 rounded-4 border border-light-subtle border-2" key={item.idMeal}>
              <img className='w-100 plateImg' src={item.strMealThumb} alt={item.strMeal} />
              <div className="text-center p-2">
                <h3 className='text-danger'>$ 20.99</h3>
                <h4>{item.strMeal}</h4>
                <p className='text-secondary-emphasis'>Made with eggs, lettuce, salt, oil and other ingredients.</p>
              </div>
            </div>
          ))}
        </div>
